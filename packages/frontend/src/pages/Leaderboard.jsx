  ...
  return (
    <div className="container animate-fade-in" style={{ paddingTop: '40px', paddingBottom: '80px' }}>
      <div style={{ textAlign: 'center', marginBottom: '40px' }}>
        <h2 className="hero-title" style={{ fontSize: '3rem', marginBottom: '16px' }}>Hall of <span className="text-gradient">Fame</span></h2>
        <p className="text-secondary" style={{ fontSize: '1.1rem' }}>Top players competing for glory in Season {seasonId}</p>
      </div>
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {CONTRACT_ADDRESS === "0x" ? (
          ...
          top10.map((score, idx) => (
            ...
            >
              <div style={{ width: '50px', fontSize: '1.5rem', fontWeight: '800', color: getRankColor(idx) }}>
                #{idx + 1}
              </div>
              
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: 'monospace', fontSize: '1.1rem', fontWeight: '600', color: 'white' }}>
                  {shortenAddress(score.player)}
                </div>
                {idx === 0 && <span className="badge badge-primary" style={{ marginTop: '6px', display: 'inline-block' }}>Champion</span>}
              </div>
              
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '1px' }}>Score</div>
                <div className="text-gradient" style={{ fontSize: '1.8rem', fontWeight: '800', fontFamily: 'Space Grotesk' }}>
                  {Number(score.score)}
                </div>
              </div>
            </div>
